...
class employe extends ConnexionDB  {
	...
	{
		$sql = $this->cnx->prepare("INSERT INTO employes (prenom,nom,email,age,ville,genre,username,mdp,typec) 
        VALUES (?,?,?,?,?,?,?,?,?)");
		$sql->execute( array($empl['prenom'],$empl['nom'],$empl['email'],$empl['age'],$empl['ville'],$empl['genre'],$empl['username'],$empl['mdp'],$empl['typec']) );
		return $sql->rowCount();
	}
